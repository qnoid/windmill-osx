...
{
xcrun git rev-parse "$BRANCH" > "$POLL_DIRECTORY_FOR_PROJECT/prev_head" || exit 0
}

file_does_not_exist_at_path "$POLL_DIRECTORY_FOR_PROJECT/prev_head" store_prev_head

xcrun git fetch 2>/dev/null || exit 0
xcrun git reset --soft origin/"$BRANCH" > /dev/null || exit 0
xcrun git rev-parse "$BRANCH" > "$POLL_DIRECTORY_FOR_PROJECT/latest_head" || exit 0
